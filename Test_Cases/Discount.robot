...
New Discount Generation
    [Arguments]   ${DIS_NAME}  ${DURATION}  ${DUR_VALUE}  ${DISC_GL_CODE}  ${DIS_ORIGIN}  ${TARGET}  ${APP_FOR}  ${COUNTED_TOWARDS}  ${SLAB_TYPE}  ${MIN_SLAB}  ${MAX_SLAB-1}  ${DIS_SLAB-1}  ${MAX_SLAB-2}  ${DIS_SLAB-2}  ${NO_SLABS}  ${MAX_SLAB-3}    ${DIS_SLAB-3}  ${NEW_DIS_VERIFY}  ${EDIT_DIS_NAME}  ${EDIT_APP_FOR}  ${EDIT_DISC_GL_CODE}  ${EDIT_DIS_VERIFY}
    New Discounts cycle
    #Re-running the random string fuction for new set of values
    Initialize Random Variables for Discounts cycle
    #inputing new data
    Give input to Discounts cycle   ${DIS_NAME}  ${DURATION}  ${DUR_VALUE}  ${DISC_GL_CODE}  ${DIS_ORIGIN}  ${TARGET}  ${APP_FOR}  ${COUNTED_TOWARDS}  ${SLAB_TYPE}  ${MIN_SLAB}  ${MAX_SLAB-1}  ${DIS_SLAB-1}  ${MAX_SLAB-2}  ${DIS_SLAB-2}  ${NO_SLABS}  ${MAX_SLAB-3}    ${DIS_SLAB-3}  ${NEW_DIS_VERIFY}
